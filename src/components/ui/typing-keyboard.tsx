...
export interface TypingKeyboardProps extends React.HTMLAttributes<HTMLDivElement> {
    ...
    loop?: boolean;
    /** Callback when typing animation finishes one cycle */
    onComplete?: () => void;
...
export function TypingKeyboard({
    ...
    const actualScreenColor = screenColor || accentColor;
    ...
    useEffect(() => {
        ...
        // Initialize screen with terminal prompt and cursor
        const resetScreen = () => {
            if (!screen) return;
            screen.innerHTML = '<span class="tk-prompt" style="color: #60a5fa;">guest@sreategui:~$ </span><span class="tk-text"></span><span class="tk-cursor" style="animation: tk-blink 1s step-end infinite;">_</span>';
        };
        resetScreen();
        ...
        const typeNext = () => {
            ...
            const textEl = screen.querySelector(".tk-text");
            if (textEl) {
    ...
    return (
        ...
        @media (max-width: 768px) {
          .tk-main {
            transform: scale(calc(${scale} * 0.75));
          }
          .tk-screen {
            font-size: 20px;
          }
        ...
        @media (max-width: 480px) {
          .tk-main {
            transform: scale(calc(${scale} * 0.65));
          }
          .tk-screen {
            font-size: 24px;
          }
        ...
        .tk-screen {
          ...
          background-color: ${actualScreenColor};
          ...
          box-shadow:
            ...
            0 0 15px color-mix(in srgb, ${secondaryAccent} 60%, transparent),
            0 0 20px color-mix(in srgb, ${secondaryAccent} 50%, transparent),
            0 0 40px color-mix(in srgb, ${secondaryAccent} 40%, transparent),
            0 0 60px color-mix(in srgb, ${secondaryAccent} 30%, transparent);
          animation: tk-screen-flicker 1s ease-in alternate infinite;
        }
        @keyframes tk-screen-flicker {
          0%, 90%, 96% { background-color: ${actualScreenColor}; }
          93%, 100%    { background-color: color-mix(in srgb, ${actualScreenColor} 80%, black); }
        }
        @keyframes tk-blink {
          0%, 100% { opacity: 1; }
          50% { opacity: 0; }
